...
## Steps for running
It requires a working installation of Julia (better works with `v1.10.10`. It also wroks with `v1.11` and `v1.12` series but you could find performance regressions for our similarity search package. 
...
<https://github.com/sisap-challenges/sisap2026-julia-example/blob/main/.github/workflows/ci.yml>

Note that you will need to adjust your scripts to hold the correct hyperparameters for any benchmark you use

### Clone this repository
```base
git clone https://github.com/sisap-challenges/sisap2026-julia-example
cd sisap2026-julia-example
```

### Fetch the datasets
You can clone our dataset repository or fetch dataset by dataset; run the `prepare-data.sh` script to download the necessary testing data. Note that we expect to use small dataset for your GitHub Actions.

[https://huggingface.co/datasets/SISAP-Challenges/SISAP2026/tree/main](https://huggingface.co/datasets/SISAP-Challenges/SISAP2026/tree/main)
...
```bash
JULIA_PROJECT=. JULIA_NUM_THREADS=8 julia -L sisap2026.jl -e 'main_task1(); main_task2()'
```

You should modify the number of threads to adapt your hardware; you can also try to add optimization flags to julia, e.g., `julia -O3 -Cnative`. 
...
<https://github.com/sisap-challenges/sisap2026-julia-example/actions>
